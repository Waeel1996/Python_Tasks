## 1. Create a Boolean variable named x :


x = True
print ('x =', type(x))



print ('--------------------------------')
# ------------------------------------------------


## 2. Create an integer variable named y :


y = 10
print ('y =', type(y))



print ('--------------------------------')
# ------------------------------------------------


## 3. Create a float variable named z :

z = 1.0
print ('z =',type(z))



print ('--------------------------------')
# ------------------------------------------------

## 4. Create a string variable names s :

s = 'Wael'
print ('s =', type(s))



print ('--------------------------------')
# ------------------------------------------------


## 5. Convert the int variable to float :

p = 11
print ('p =', type(p))
p = float (p)
print ('p has been converted to',type(p))



print ('--------------------------------')
# ------------------------------------------------


## 6. Can we convert the str to int ?

h = '21'
print (int(h), type(h))
h = int = 1
print ((1),'is now the new vlaue of h = :',type(h))



print ('--------------------------------')
# ------------------------------------------------


## 7. Create a list of numbers from 1 to 5 :

l = [1,2,3,4,5]
print ('l =',(l),type(l))



print ('--------------------------------')
# ------------------------------------------------


## 8. Create a tuple from 10 to 15 :

t = (10, 11, 12, 13, 14, 15)
print('t =',(t), type(t))


print ('--------------------------------')
# ------------------------------------------------


## 9. Convert the list to tuple :

l = [1,2,3,4,5]
print ('l =',(l),type(l))

t = (10, 11, 12, 13, 14, 15)
print('t =',(t), type(t))

l = tuple(l)
print('l has been coverted to tuple =',(l),type(l))


print ('--------------------------------')
# ------------------------------------------------


## 10.Create a dict of 3 values

d = {'name': 'judy','age': 21, 'hight': 179.5}
print ('d =',(d),type(d))


print ('--------------------------------')
# ------------------------------------------------


## 11. Can we use semi colon ; with python

print ("yes, but Python does not require semi-colons to terminate statements. Semicolons can be used to delimit statements if you wish to put multiple statements on the same line.")


print ('--------------------------------')
# ------------------------------------------------

## 12. Python is interpreted or compiled ?

print("Python is an interpreted language, which means the source code of a Python program is converted into bytecode that is then executed by the Python virtual machine ")


print ('--------------------------------')
# ------------------------------------------------

## 13. What is the differences between low level & high level programing language ?

print ('High-level languages require the use of a compiler or an interpreter for their translation into the machine code. Low-level language requires an assembler for directly translating the instructions of the machine language. These languages have a very low memory efficiency.')


print ('--------------------------------')
# ------------------------------------------------
